...
if (isset($_POST['add_announcement'])) {
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO announcements (user_id, title, content, image_path) VALUES (?, ?, ?, ?)");
        $stmt->execute([$user_id, $title, $content, $image_path]);
        // Log add (readable format)
        $new_str = '';
        foreach (['title' => $title, 'content' => $content, 'image_path' => $image_path] as $k => $v) {
            if ($v) $new_str .= "$k: $v\n";
        }
        log_activity_db(
            $pdo,
...
if (isset($_POST['update_announcement'])) {
    ...
    try {
        ...
        if ($image_path) {
            $stmt = $pdo->prepare("UPDATE announcements SET title = ?, content = ?, image_path = ? WHERE id = ?");
