...
/**
 * Gmail SMTP Sender
 * Direct connection to Gmail SMTP server - FREE FOREVER
 */
...
class GmailSMTPSender {
    ...
    public function sendPasswordResetEmail($user_email, $user_name, $reset_link) {
        if (!$this->is_configured) {
            error_log('Gmail SMTP sender is not configured: required SMTP credentials are missing.');
            return false;
        }
    ...
    private function sendEmail($to_email, $subject, $message) {
        try {
            // Create SSL connection to Gmail SMTP
...
/**
 * Simple function to send password reset email via Gmail SMTP
 */
function sendPasswordResetEmailViaGmailSMTP($user_email, $user_name, $reset_link) {
